...
import { 
  ...
  FileText, Map as MapIcon, Clock, Upload, BarChart2, Utensils, Timer, Hourglass, Layers
} from 'lucide-react';
...
import EngineRoomPage from './EngineRoomPage';
...
const AdminPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'REQUESTS' | 'USERS' | 'POSTS' | 'GALLERY' | 'EVENTS' | 'ROLLOVER' | 'SETTINGS' | 'ENGINE_ROOM' | 'ASSETS' | 'SURF_CALLS'>('USERS');
  ...
  const adminTabs = [
    ...
    { id: 'SETTINGS', label: 'הגדרות מערכת', icon: <Settings size={20} /> },
    { id: 'ENGINE_ROOM', label: 'חדר המכונות', icon: <Terminal size={20} /> },
      ...
      <div className="max-w-7xl mx-auto relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in zoom-in-95 duration-500">
                {[
                  ...
                  ...(isAdmin ? [
                    ...
                    { id: 'SETTINGS', label: 'הגדרות מערכת', desc: 'קונפיגורציית ליבה ומועדים', icon: Settings, color: 'bg-[#8B5CF6]' },
                    { id: 'ENGINE_ROOM', label: 'חדר המכונות', desc: 'ניטור תשתיות ומערכות', icon: Terminal, color: 'bg-[#00AFC2]' },
        ...
        {activeTab === 'SETTINGS' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <SystemSettingsPage embedded={true} />
          </div>
        )}

        {activeTab === 'ENGINE_ROOM' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <EngineRoomPage embedded={true} />
          </div>
        )}
