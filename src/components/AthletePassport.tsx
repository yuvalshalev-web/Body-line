...
// Define the available disciplines/sports with luxury ocean and deep atmospheric gradients
const DISCIPLINES = [
  { id: 'surfing', name: 'גלישת גלים', icon: Waves, color: 'from-[#002f4a] via-[#003b5c] to-[#004e75]', shadow: 'shadow-[0_20px_50px_rgba(0,59,92,0.4)]' },
  { id: 'sailing', name: 'סקיפר / שייט', icon: Anchor, color: 'from-[#0b1b36] via-[#12284c] to-[#071326]', shadow: 'shadow-indigo-950/50' },
  { id: 'diving', name: 'צלילה', icon: Waves, color: 'from-[#002b2b] via-[#003d3d] to-[#001f1f]', shadow: 'shadow-teal-950/50' },
  { id: 'climbing', name: 'טיפוס הרים', icon: Mountain, color: 'from-[#2d1810] via-[#3d2015] to-[#1c0f0a]', shadow: 'shadow-orange-950/50' },
  { id: 'rock-climbing', name: 'טיפוס צוקים', icon: Mountain, color: 'from-[#202428] via-[#2c3238] to-[#16191c]', shadow: 'shadow-stone-950/50' },
  { id: 'ski', name: 'סקי / סנובורד', icon: Snowflake, color: 'from-[#1b263b] via-[#293b59] to-[#0f1724]', shadow: 'shadow-slate-900/50', textColor: 'text-white' },
  { id: 'golf', name: 'גולף', icon: Flag, color: 'from-[#062416] via-[#0b3823] to-[#03170e]', shadow: 'shadow-emerald-950/50' },
  { id: 'paragliding', name: 'מצנח רחיפה', icon: Wind, color: 'from-[#0a233f] via-[#10345d] to-[#05172b]', shadow: 'shadow-sky-950/50' },
  { id: 'canyoning', name: 'קניונינג', icon: MapIcon, color: 'from-[#2d1b09] via-[#42270d] to-[#1a1005]', shadow: 'shadow-amber-950/50' },
  { id: 'rafting', name: 'רפטינג', icon: Ship, color: 'from-[#002b30] via-[#003e45] to-[#001c20]', shadow: 'shadow-teal-950/50' },
  { id: 'skydiving', name: 'צניחה חופשית', icon: Wind, color: 'from-[#0a2342] via-[#123661] to-[#06182e]', shadow: 'shadow-blue-950/50' },
  { id: 'all', name: 'הכל', icon: Award, color: 'from-[#0f172a] via-[#1e293b] to-[#020617]', shadow: 'shadow-black/60' }
];

const PassportStamp = ({ club, location, date, colorClass, rotateClass, icon: Icon }: any) => {
  return (
    <div className={`relative w-[85px] h-[85px] rounded-full border-[3px] border-dashed ${colorClass} ${rotateClass} flex flex-col items-center justify-center p-1 opacity-90 hover:opacity-100 transition-all duration-300 hover:scale-[1.15] cursor-help shadow-md group/stamp backdrop-blur-xs`}>
       ...
       <Icon size={14} className="mb-0.5 opacity-90" />
       ...
       <span className="font-bold text-[7px] uppercase tracking-widest mt-0.5 opacity-90">{location}</span>
       <span className="font-mono text-[8px] font-black mt-1 opacity-95">{date}</span>
       ...
       {/* Tooltip */}
       <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/stamp:opacity-100 transition-opacity bg-slate-950 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg whitespace-nowrap z-[200] shadow-xl border border-white/20 pointer-events-none">
          {club} - {location} ({date})
       </div>
    </div>
  )
...
const getClubStamps = (discipline: string, memberId: string) => {
  switch (discipline) {
    case 'surfing':
      return [
        { id: 1, club: "ריף הרצליה", location: "IL", date: "12 AUG 2023", colorClass: "text-cyan-300 border-cyan-300", rotateClass: "-rotate-[14deg]", icon: Waves },
        { id: 2, club: "Hanalei Bay", location: "North Shore, HI", date: "04 NOV 2024", colorClass: "text-amber-300 border-amber-300", rotateClass: "rotate-[9deg]", icon: Wind }
      ];
    case 'diving':
      return [
        { id: 3, club: "Manta Ray Bay", location: "Palau", date: "22 JAN 2024", colorClass: "text-emerald-300 border-emerald-300", rotateClass: "rotate-[15deg]", icon: Waves },
        { id: 4, club: "מרינה דייוורס", location: "אילת", date: "15 MAR 2025", colorClass: "text-teal-300 border-teal-300", rotateClass: "-rotate-[7deg]", icon: Waves }
      ];
    case 'sailing':
      return [
        { id: 5, club: "Yacht Club", location: "Monaco", date: "05 SEP 2023", colorClass: "text-sky-300 border-sky-300", rotateClass: "-rotate-[22deg]", icon: Anchor }
      ];
     case 'skydiving':
      return [
        { id: 6, club: "Skydive Dubai", location: "UAE", date: "10 FEB 2024", colorClass: "text-amber-300 border-amber-300", rotateClass: "rotate-[12deg]", icon: Wind },
        { id: 7, club: "פרדייב", location: "הבונים", date: "01 MAY 2025", colorClass: "text-cyan-300 border-cyan-300", rotateClass: "-rotate-[12deg]", icon: Wind }
      ];
    default:
      return [
        { id: 8, club: "מועדון הבית", location: "ישראל", date: "01 JAN 2026", colorClass: "text-cyan-300 border-cyan-300", rotateClass: "-rotate-[5deg]", icon: ShieldCheck }
      ];
  }
}
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8" dir="rtl">
      ...
      <div className="flex [perspective:1000px] justify-center">
        <motion.div
          ...
          className="w-full max-w-[420px] h-auto min-h-[680px] flex flex-col rounded-[2.5rem] relative cursor-pointer border border-cyan-400/30 shadow-[0_25px_60px_rgba(0,47,74,0.45)] backdrop-blur-xl"
        >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* Ambient Glowing Orbs matching the bottom banner */}
              <div className="absolute -top-12 -right-12 w-48 h-48 bg-cyan-400/20 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute -bottom-12 -left-12 w-48 h-48 bg-amber-400/15 rounded-full blur-3xl pointer-events-none" />
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-white/5 to-transparent opacity-40 mix-blend-overlay" />
              <div className="absolute -top-[50%] -left-[50%] w-[200%] h-[200%] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.2)_0%,transparent_50%)] animate-spin-slow pointer-events-none mix-blend-overlay" style={{ animationDuration: '25s' }} />
              {/* Noise Texture */}
              <div className="absolute inset-0 opacity-15 mix-blend-overlay" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.8%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E")' }} />
            </motion.div>
          </AnimatePresence>
          ...
          >
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                <div className="flex items-center gap-1.5 bg-amber-400/20 text-amber-300 backdrop-blur-md px-3 py-1 rounded-full border border-amber-400/30 shadow-inner mb-2 inline-flex">
                  <ShieldCheck size={14} className="text-amber-300 drop-shadow-sm" />
                  <span className="text-[10px] font-black uppercase tracking-widest text-amber-200 drop-shadow-sm">Verified ID</span>
                </div>
                <h3 className="text-sm font-black text-cyan-100 uppercase tracking-widest">{activeProfile.name}</h3>
              </div>
              <div className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center border border-cyan-400/30 shadow-inner text-cyan-200">
            ...
            <div className="flex items-center gap-4 mb-10">
              <div className="w-20 h-20 rounded-2xl bg-white/10 backdrop-blur-xl border border-cyan-400/30 shadow-inner overflow-hidden p-1 flex-shrink-0">
                {member.avatar ? (
              ...
              <div>
                <h1 className="text-2xl font-black tracking-tight drop-shadow-md leading-none mb-1 text-white">
                  {member.firstName} {member.lastName}
                </h1>
                <p className="text-sm font-bold text-cyan-200/90 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-4 mb-auto">
              <div className="bg-white/[0.08] backdrop-blur-md rounded-2xl p-4 border border-cyan-400/20 shadow-inner flex flex-col">
                <span className="text-[11px] font-bold uppercase tracking-widest text-cyan-200/80 mb-1 flex items-center gap-1">
                  <Clock size={12} className="text-cyan-300" /> {activeDiscipline === 'skydiving' ? 'צניחות' : 'שעות מעשיות'}
                </span>
              ...
              <div className="bg-white/[0.08] backdrop-blur-md rounded-2xl p-4 border border-cyan-400/20 shadow-inner flex flex-col">
                <span className="text-[11px] font-bold uppercase tracking-widest text-cyan-200/80 mb-1 flex items-center gap-1">
                  <Award size={12} className="text-amber-300" /> רמת מיומנות
                </span>
                <span className="text-lg leading-tight font-black drop-shadow-md mt-auto text-white">
            ...
            <div className="mt-8 pt-6 border-t border-cyan-400/20">
              <span className="text-[11px] font-bold uppercase tracking-widest text-cyan-200/80 mb-3 block">הסמכות מאומתות</span>
                ...
                {(() => {
                  ...
                  if (Array.isArray(member.certifications) && member.certifications.length > 0) {
                    return member.certifications.map((cert: string, i: number) => (
                      <div key={i} className="px-3 py-1.5 bg-white/[0.08] backdrop-blur-md border border-cyan-400/25 rounded-lg text-xs font-bold text-cyan-100 shadow-inner flex shrink-0 items-center">
                        {cert}
                      </div>
                    ));
                  }
                  ...
                  return (
                    <div className="px-3.5 py-1.5 bg-white/[0.05] backdrop-blur-md border border-cyan-400/20 rounded-lg text-xs font-bold text-cyan-200/70">
                      אין הסמכות רשומות
                    </div>
                  );
                })()}
            ...
            {['diving', 'sailing', 'paragliding', 'ski', 'skydiving'].includes(activeDiscipline) && (
              <div className="mt-4 pt-4 border-t border-cyan-400/20">
                 <span className="text-[11px] font-bold uppercase tracking-widest text-cyan-200/80 mb-3 flex items-center gap-1"><ShieldCheck size={12}/> כיסוי ביטוחי & אישורים רשמיים</span>
                 <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between px-3 py-2 bg-white/[0.08] backdrop-blur-md rounded-lg border border-cyan-400/20 text-xs">
                    ...
                    {activeDiscipline === 'diving' && (
                      <div className="flex items-center justify-between px-3 py-2 bg-white/[0.08] backdrop-blur-md rounded-lg border border-cyan-400/20 text-xs shadow-inner">
                        <div className="flex items-center gap-2">
            ...
            <div className="mt-6 pt-5 border-t border-cyan-400/20">
              <span className="text-[11px] font-bold uppercase tracking-widest text-cyan-200/80 mb-4 flex items-center gap-1 justify-center"><VerifiedIcon size={12}/> זיהוי מועדונים בינלאומיים</span>
              <div className="flex flex-wrap items-center justify-center gap-4 px-2">
                 ...
                 {activeDiscipline === 'all' && (
                    <div className="text-[10px] text-cyan-200/60 font-medium text-center w-full">בחר ענף ספורט ספציפי לצפייה בחותמות מועדונים</div>
                 )}
              </div>
            </div>
