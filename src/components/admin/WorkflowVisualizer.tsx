      ...
      <div className="relative z-10 flex flex-col gap-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-8 relative py-8">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 w-full md:w-1/3">
            <div className="w-24 h-24 rounded-[2rem] bg-[#fcfcfd] shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100/50 flex items-center justify-center relative group">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity" />
              <Bot size={40} className="text-[#006699] relative z-10" strokeWidth={2} />
              <div className="absolute -top-2 -right-2 w-6 h-6 bg-[#7cfc00]/20 rounded-full border-2 border-[#fcfcfd] flex items-center justify-center">
                <CheckCircle2 size={12} className="text-[#7cfc00]" />
              </div>
            </div>
            <div className="text-center">
              <h3 className="text-[#003366] font-black text-lg uppercase tracking-widest">AI Studio</h3>
              <p className="text-[11px] font-black text-[#006699] uppercase tracking-widest mt-1">Code Generation</p>
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 w-full md:w-1/3">
            <div className="w-24 h-24 rounded-[2rem] bg-[#fcfcfd] shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100/50 flex items-center justify-center relative group">
              <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-gray-100 rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity" />
              <Github size={40} className="text-[#003366] relative z-10" strokeWidth={2} />
              <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full border-2 border-[#fcfcfd] flex items-center justify-center ${
                githubStatus === 'synced' ? 'bg-[#7cfc00]/20' : 
                githubStatus === 'building' ? 'bg-[#006699]/20' : 'bg-[#c94a4a]/20'
              }`}>
                {githubStatus === 'synced' && <CheckCircle2 size={12} className="text-[#7cfc00]" />}
                {githubStatus === 'building' && <RefreshCw size={12} className="text-[#006699] animate-spin" />}
                {githubStatus === 'error' && <AlertCircle size={12} className="text-[#c94a4a]" />}
              </div>
            </div>
            <div className="text-center">
              <h3 className="text-[#003366] font-black text-lg uppercase tracking-widest">GitHub</h3>
              <p className="text-[11px] font-black text-[#006699] uppercase tracking-widest mt-1">Version Control</p>
              <div className={`mt-3 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full ${
                githubStatus === 'synced' ? 'bg-[#7cfc00]/20' : 
                githubStatus === 'building' ? 'bg-[#006699]/10' : 'bg-[#c94a4a]/10'
              }`}>
                <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${
                  githubStatus === 'synced' ? 'bg-[#7cfc00]' : 
                  githubStatus === 'building' ? 'bg-[#006699]' : 'bg-[#c94a4a]'
                }`} />
                <span className={`text-[8px] font-black uppercase tracking-widest ${
                  githubStatus === 'synced' ? 'text-[#7cfc00]' : 
                  githubStatus === 'building' ? 'text-[#006699]' : 'text-[#c94a4a]'
                }`}>
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 w-full md:w-1/3">
            <div className="w-24 h-24 rounded-[2rem] bg-[#fcfcfd] shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100/50 flex items-center justify-center relative group">
              <div className="absolute inset-0 bg-gradient-to-br from-slate-900 to-black rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity" />
              <svg width="40" height="40" viewBox="0 0 76 65" fill="currentColor" className="text-[#003366] group-hover:text-white transition-colors relative z-10">
                <path d="M37.5274 0L75.0548 65H0L37.5274 0Z" />
              </svg>
              <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full border-2 border-[#fcfcfd] flex items-center justify-center ${
                vercelStatus === 'ready' ? 'bg-[#7cfc00]/20' : 
                vercelStatus === 'building' ? 'bg-[#006699]/20' : 'bg-[#c94a4a]/20'
              }`}>
                {vercelStatus === 'ready' && <CheckCircle2 size={12} className="text-[#7cfc00]" />}
                {vercelStatus === 'building' && <Activity size={12} className="text-[#006699] animate-pulse" />}
                {vercelStatus === 'error' && <AlertCircle size={12} className="text-[#c94a4a]" />}
              </div>
            </div>
            <div className="text-center">
              <h3 className="text-[#003366] font-black text-lg uppercase tracking-widest">Vercel</h3>
              <p className="text-[11px] font-black text-[#006699] uppercase tracking-widest mt-1">Production Hosting</p>
              <div className={`mt-3 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full ${
                vercelStatus === 'ready' ? 'bg-[#7cfc00]/20' : 
                vercelStatus === 'building' ? 'bg-[#006699]/10' : 'bg-[#c94a4a]/10'
              }`}>
                <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${
                  vercelStatus === 'ready' ? 'bg-[#7cfc00]' : 
                  vercelStatus === 'building' ? 'bg-[#006699]' : 'bg-[#c94a4a]'
                }`} />
                <span className={`text-[8px] font-black uppercase tracking-widest ${
                  vercelStatus === 'ready' ? 'text-[#7cfc00]' : 
                  vercelStatus === 'building' ? 'text-[#006699]' : 'text-[#c94a4a]'
                }`}>
