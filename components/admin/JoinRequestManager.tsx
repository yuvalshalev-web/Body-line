    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="relative">
        ...
        <input 
          ...
          className="w-full pr-16 pl-6 py-6 luxury-card rounded-[2.5rem] border-none font-black focus:ring-2 ring-[#ff009f]/30 shadow-sm"
          value={searchTerm}
      ...
      {filteredRequests.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRequests.map(req => (
            <div key={req.id} className={`luxury-slab rounded-[3rem] p-8 transition-all group flex flex-col h-full ${isProcessing === req.id ? 'opacity-50' : ''}`}>
              <div className="flex items-start gap-5 mb-8">
                {req.avatar ? (
                  <img src={req.avatar} className="w-16 h-16 rounded-2xl object-cover shadow-md" alt="" />
                ) : (
                  <div className="w-16 h-16 rounded-2xl luxury-card flex items-center justify-center text-[#ff009f] shadow-md">
                    <UserCircle size={32} />
                  </div>
                )}
                <div>
              ...
              <div className="space-y-4 flex-1">
                <div className="flex items-center gap-3 p-3 luxury-card rounded-xl">
                ...
                <div className="flex items-center gap-3 p-3 luxury-card text-[#ff009f] rounded-xl">
                  <MapPin size={14} />
              ...
              <div className="flex gap-3 mt-10">
                <button 
                  ...
                  className="flex-1 py-4 bg-indigo-500 text-white rounded-2xl font-black text-sm hover:bg-indigo-600 transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
                >
        ...
        <div className="py-32 text-center luxury-slab rounded-[4rem]">
          <div className="w-20 h-20 luxury-card rounded-full flex items-center justify-center mx-auto mb-6 text-[#f063c1]/20">
            <UserCheck size={40} />
          </div>
